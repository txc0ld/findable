...
const PREFIXES = [
  "nexus","cipher","phantom","sentinel","vector","cortex","flux","arc",
  "helix","node","prism","qubit","relay","synth","vertex","axiom",
  "delta","echo","forge","grid","halo","iris","jolt","lumen","matrix",
  "nova","orbit","pulse","radix","sigma","tensor","umbra","vortex",
  "warp","xenon","zero","apex","bolt","crypt","drift","ember","frost",
  "glyph","hydra","ion","jade","kilo","lyra","morph","neon","oxide",
  "quasar","rune","shard","thorn","ultra","vale","wraith","yield",
];
const SUFFIXES = ["core","mind","link","net","sys","ops","hub","lab","dev","run","gen","mod","sec","ai"];
...
const TH = {
  dark: {
    bg: 0x000000, cssBg: "#000000",
    node: 0xffffff,
    edge: 0xffffff, edgeOp: 0.18,
    text: "#fff", textDim: "rgba(255,255,255,0.35)",
    uiBg: "rgba(255,255,255,0.04)", uiBdr: "rgba(255,255,255,0.08)",
    togOn: "#fff", togOnTxt: "#000", togOff: "transparent", togOffTxt: "rgba(255,255,255,0.3)",
  },
  light: {
    bg: 0xf5f5f5, cssBg: "#f5f5f5",
    node: 0x111111,
    edge: 0x000000, edgeOp: 0.18,
    text: "#000", textDim: "rgba(0,0,0,0.35)",
    uiBg: "rgba(0,0,0,0.03)", uiBdr: "rgba(0,0,0,0.08)",
    togOn: "#000", togOnTxt: "#fff", togOff: "transparent", togOffTxt: "rgba(0,0,0,0.3)",
  },
};
...
export default function AgntNetwork() {
  ...
  useEffect(() => {
    ...
    function addNode() {
      ...
      const mat = new THREE.MeshBasicMaterial({ color: tm.node, transparent: true, opacity: 0 });
    ...
    function addEdge(aIdx, bIdx) {
      ...
      scene.add(mesh);
      graph.edges.push({ a: aIdx, b: bIdx, line: mesh, mat, geo: null, birthTime: Date.now(), fadeIn: 0 });
    }
    ...
    function connectNewNode(newIdx) {
      ...
      if (Math.random() < 0.25 && graph.nodes.length > 4) {
        ...
        let rB = Math.floor(Math.random() * graph.nodes.length);
    ...
    const onMD = (e) => { orb.drag = true; orb.lx = e.clientX; orb.ly = e.clientY; };
    const onMU = () => { orb.drag = false; };
    const onMM = (e) => {
      if (!orb.drag) return;
      orb.th -= (e.clientX - orb.lx) * 0.005;
      orb.ph = Math.max(0.3, Math.min(Math.PI - 0.3, orb.ph - (e.clientY - orb.ly) * 0.005));
      orb.lx = e.clientX; orb.ly = e.clientY;
    };
    const onWH = (e) => { e.preventDefault(); cam.position.multiplyScalar(e.deltaY > 0 ? 1.06 : 0.94); };
    el.addEventListener("mousedown", onMD);
    window.addEventListener("mouseup", onMU);
    window.addEventListener("mousemove", onMM);
    el.addEventListener("wheel", onWH, { passive: false });

    const onTS = (e) => { if (e.touches.length === 1) { orb.drag = true; orb.lx = e.touches[0].clientX; orb.ly = e.touches[0].clientY; } };
    const onTE = () => { orb.drag = false; };
    const onTMv = (e) => {
      if (!orb.drag || e.touches.length !== 1) return;
      orb.th -= (e.touches[0].clientX - orb.lx) * 0.005;
      orb.ph = Math.max(0.3, Math.min(Math.PI - 0.3, orb.ph - (e.touches[0].clientY - orb.ly) * 0.005));
  ...
  return (
    <div style={{
      ...
      background: tm.cssBg, overflow: "hidden", cursor: "grab",
      ...
      <div style={{ position: "absolute", top: 24, left: 24, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{
            ...
            color: tm.text, textTransform: "uppercase",
          }}>.agnt protocol</span>
        </div>
        <div style={{
          fontSize: 9, letterSpacing: "0.1em", color: tm.textDim,
          marginTop: 4, paddingLeft: 17, textTransform: "uppercase",
        }}>agent network topology · live</div>
      </div>
      ...
      <div style={{
        ...
        background: tm.uiBg, border: `1px solid ${tm.uiBdr}`,
      }}>
        {["dark", "light"].map((m) => (
        ...
        {[
          ...
          <div key={s.k} style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            ...
            <span style={{
              ...
              color: tm.textDim, textTransform: "uppercase",
            }}>{s.k}</span>
          </div>
        ))}
      </div>
      ...
      {hover && (
        <div style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: PULSE_CSS, letterSpacing: "0.03em" }}>
            {hover.name}
          </div>
          <div style={{ fontSize: 8, color: tm.textDim, letterSpacing: "0.08em", marginTop: 2 }}>
